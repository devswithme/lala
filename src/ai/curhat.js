...
/**
 * Build the Lala system prompt using the user's profile data.
 */
 ...
 */
export async function chat({ user, userText }) {
  ...
  const reply = res.choices[0]?.message?.content?.trim() ?? "Hmm, Lala lagi nggak bisa mikir sekarang. Coba lagi ya!";
